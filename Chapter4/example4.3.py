...
print('Example 4.3 Cooling of a circular fin by means of convective heat transfer along its length')
print('-----------------------------------------------------------------------')
...
for i in range(n):
    if i == 0:                # Exterior Node (Node 1)
        x[i] = delta_x / 2
        sol[i] = math.cosh(n_square ** 0.5 * (L - x[i])) * (T_B - T_ambient) / math.cosh(
            n_square ** 0.5 * L) + T_ambient
        a_W[i] = 0
        a_E[i] = 1 / delta_x
        S_u = np.vstack((S_u, n_square * delta_x * T_ambient + 2 * T_B / delta_x))
        S_u_arr[i] = n_square * delta_x * T_ambient + 2 * T_B / delta_x
        S_P[i] = -n_square * delta_x - 2 / delta_x
        ...
        a_W[i] = 1 / delta_x
        a_E[i] = 1 / delta_x
        S_u = np.vstack((S_u, n_square * delta_x * T_ambient))
        S_u_arr[i] = n_square * delta_x * T_ambient
        S_P[i] = -n_square * delta_x
        a_P[i] = a_W[i] + a_E[i] - S_P[i]
        m = np.zeros(n)
        m[i-1] = -a_W[i]
        m[i] = a_P[i]
...
data1 = {'Node': nodes, 'a_W': a_W, 'a_E': a_E, 'S_u': S_u_arr,
         'S_P': S_P, 'a_P=a_W+a_E-S_P': a_P}
data2 = {'Node': nodes, 'x(m)': x, 'Finite volume solution': T_arr,
...
plt.title("Cooling of a circular fin by convective heat transfer")
