...
print('Example 4.2 1-D Steady Conduction with heat source')
print('-----------------------------------------------------------------------')
...
# Constants
k = 0.5         # Watt per meter Kelvin
A = 1           # meter square
L = 0.02        # meter
q = 1000000     # Watt per meter cubic, multiple 1e-03 to change into kiloWatt per meter cubic
T_A = 100       # deg Celsius
T_B = 200       # deg Celsius
...
for i in range(n):
    if i == 0:                # Exterior Node (Node 1)
        x[i] = delta_x / 2
        sol[i] = ((T_B-T_A) / L + q * (L - x[i]) / (2 * k)) * x[i] + T_A
        a_W[i] = 0
        a_E[i] = k * A / delta_x
        S_u = np.vstack((S_u, q * A * delta_x + 2 * k * A * T_A / delta_x))
        S_u_arr[i] = q * A * delta_x + 2 * k * A * T_A / delta_x
        S_P[i] = -2 * k * A / delta_x
        ...
        a_W[i] = k * A / delta_x
        a_E[i] = k * A / delta_x
        S_u = np.vstack((S_u, q * A * delta_x))
        S_u_arr[i] = q * A * delta_x
        S_P[i] = 0
        a_P[i] = a_W[i] + a_E[i] - S_P[i]
        m = np.zeros(n)
        m[i-1] = -a_W[i]
        ...
        a_W[i] = k * A / delta_x
        a_E[i] = 0
        S_u = np.vstack((S_u, q * A * delta_x + 2 * k * A * T_B / delta_x))
        S_u_arr[i] = q * A * delta_x + 2 * k * A * T_A / delta_x
        S_P[i] = -2 * k * A / delta_x
        a_P[i] = a_W[i] + a_E[i] - S_P[i]
        m = np.zeros(n)
...
y_sol = ((T_B - T_A) / L + q * (L - x_sol) / (2 * k)) * x_sol + T_A
...
plt.title("1-D Steady Conduction with heat source")
